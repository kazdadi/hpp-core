...
namespace hpp {
  namespace core {
	...
	return shPtr;
      ...
      {
        value_type alphaInit = problem().getParameter ("SplineGradientBasedConstraint/alphaInit", value_type(0.2));
        bool alwaysStopAtFirst = problem().getParameter ("SplineGradientBasedConstraint/alwaysStopAtFirst", true);
        bool returnOptimum = problem().getParameter ("SplineGradientBasedConstraint/returnOptimum", false);
        value_type costThreshold = problem().getParameter ("SplineGradientBasedConstraint/costThreshold", value_type(0.01));
        ...
        while (!(noCollision && minimumReached) && (!this->interrupt_)) {
          ...
          if (computeInterpolatedSpline) {
            ...
            applyConstraints(alphaSplines, alphaSplines, input->constraints());
      ...
      template <int _PB, int _SO>
      void SplineGradientBasedConstraint<_PB, _SO>::interpolate
      ...
      template <int _PB, int _SO>
      void SplineGradientBasedConstraint<_PB, _SO>::applyConstraints
      (const Splines_t& a, Splines_t& res, ConstraintSetPtr_t constraints) const
      {
        assert (a.size() == res.size());

        for (std::size_t i = 0; i < a.size(); ++i)
          res[i] = Spline(a[i], constraints);
      }
